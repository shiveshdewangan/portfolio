    ...
    <div className="text-white font-mont fixed top-0 left-0 right-0 z-50">
      <div className={`flex bg-theme justify-between items-center p-2 shadow-lg ${showMenu === "" && "md:flex-col"}`}>
        <div className="flex justify-between items-center w-full">
          <Link to="/">
            <h1 className="text-4xl font-semibold hover:text-yellow-500 cursor-pointer">SHIV DEV</h1>
          </Link>
          <FaBars
            onClick={() => {
        ...
        <div className="flex md:hidden">
          {menuItems.map(item => {
            return (
              <li className={`list-none mx-5 px-5 ${item.key === pathname && "bg-white text-black rounded-md"}`}>
        ...
        <div
          ...
          {menuItems.map(item => {
            return (
              <li className={`list-none mt-5 ${item.key === pathname && "bg-white text-black rounded-md"}`}>
                <Link to={`${item.key}`}>{item.title}</Link>
              </li>
            );
          })}
